...
# Routes de diagnostic pour voir si l'API marche dans le navigateur
@app.get("/")
@app.get("/api")
@app.get("/api/")
@app.get("/api/index")
async def root():
    return {"status": "ok", "message": "✅ L'API backend Vercel fonctionne parfaitement. Tu dois utiliser l'interface HTML à la racine pour uploader des fichiers."}

# Routes d'upload (accepte /api/upload ET /upload au cas où Vercel coupe le préfixe)
@app.post("/api/upload")
@app.post("/upload")
